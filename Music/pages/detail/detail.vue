<template>
	<view class="detail">
		...
		<musichead :title='songDetail.al.name' :icon='true' color="white"></musichead>
		<view class="container" v-show="!isLoading">
			<scroll-view scroll-y="true" >
				...
				<view class="detail-comment" >
					<view class="detaile-comment-head">精彩评论</view>
...
<script>
	...
	export default {
		...
		methods: {
			getMusic(songIds){
				this.$store.commit('NEXT_ID', songIds)
				console.log(this.$store.state.nextId)
				uni.showLoading({
					title:'正在加载...'
				});
				this.isLoading = true
				Promise.all([songDetail(songIds), songSimi(songIds), songComment(songIds), songLyric(songIds), songUrl(songIds)]).then((res)=>{
					// console.log(res)
					...
					if(res[4][1].data.code == '200'){
						...
						this.bgAudioMannager.title = this.songDetail.al.name
						// #endif
...
<style scoped>
	.detail-play{
		...
		background-size: cover;  //图片适用容器大小
		margin: 214rpx 80rpx auto;
		position: relative;
	}
	.detail-play image{
		width: 370rpx;
	...
	.detail-comment-item image{width: 100%; height: 100%;}
	...
	.detail-comment-name view:nth-child(2){font-size: 18rpx;}
